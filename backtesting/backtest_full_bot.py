...
async def backtest_full_bot(
    ...
    for cycle in range(n_cycles):
        ...
        if debug:
            ...
            print(f"CYCLE {cycle}: Balance = {perf.get('balance', 0):.2f}, Win Rate = {perf.get('win_rate', 0):.2%}")
...
if __name__ == "__main__":
    # Charger tes historiques (ex: Binance CSV)
    ...
    # Optionnel: charger des news simulées
    news_data = [] # ou charger d'un fichier

    # Lancer le backtest fidèle
    results, market_data, trade_decisions = asyncio.run(
        backtest_full_bot(historical_data, news_data, n_cycles=300, capital_start=10000.0, timeframes=["1h"], debug=True)
    )
